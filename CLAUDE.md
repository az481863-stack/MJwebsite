...
- **後記(2026-07-01):聊天機器人「無法回覆」除錯性強化**(commit `abcad52`)
  ...
  - **小幫手對話後台 + IP 封鎖(同日)**:教授要求可在後台檢視訪客與前台小幫手的對話,並能關掉特定 IP 的小幫手。**這反轉了階段七「對話不存 DB」的刻意取捨** → 新增 `ChatLog`(留存每則訊息)與 `IpBlock`(封鎖清單)兩表(migration `add_chat_logs_ip_block`)、`src/lib/chatlog.ts`(留存/查詢/封鎖/清理)。
    ...
    - **封鎖生效**:root `layout.tsx` 讀訪客 IP、`isIpBlocked` 命中則不掛 `ChatWidget`;`/api/chat` 亦擋(403,雙保險)。留存於 `/api/chat`:記使用者訊息 + 串流結束後記完整回覆(fire-and-forget、內建 try/catch,不影響聊天)。
    - **隱私/保留**:對話為個資,**90 天由 `/api/cron` 自動清理**(`purgeOldChatLogs`);IP 為弱識別(NAT 多人共用、會變動),封鎖屬**軟性勸退**非可靠存取控制,後台頁已明示此限制。日期運算一律以台灣 UTC+8 當地日計(`taiwanDayRangeUtc`)。
...
- **管理員小幫手(Emerald 綠 `#059669`)**:
  - 掛在 `admin/layout.tsx`,**出現在所有後台頁**(`AdminChatWidget.tsx`,client);**知識來源 = `SiteSettings.adminGuide` 全文**(量小,直接放 system prompt,**無 function calling**)。顯示條件:`isAiEnabled()` 且 `roleAtLeast(me.role,"ADMIN")`(儀器負責人若為學生不顯示,因 route 也擋 ADMIN)。
...
- **前台實驗室小幫手只出現在前台公開頁**:`ChatWidget.tsx` 以 `usePathname()` 判斷,凡 `/admin`、`/account`、`/login`、`/setup`、`/invite`、`/auth` 等後台/會員/登入頁一律 `return null`(hooks 之後、JSX 之前 early return);後台改由綠色管理員小幫手負責。兩者視覺明確區隔(前台青色 `--accent` band-dark vs 後台淺色 + Emerald)。
